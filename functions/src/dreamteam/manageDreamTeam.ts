...
export const saveDreamTeamSettings = onCall(async (request) => {
  if (!request.auth?.token?.admin) {
    throw new HttpsError("permission-denied", "Only administrators can update Dream Team settings.");
  }
...
export const publishDreamTeamRankings = onCall(
  { timeoutSeconds: 300 },
  async (request) => {
    if (!request.auth?.token?.admin) {
      throw new HttpsError("permission-denied", "Only administrators can publish rankings.");
    }

    const { rankings, phase } = request.data;

    if (!rankings || typeof rankings !== "object") {
      throw new HttpsError("invalid-argument", "rankings object is required.");
    }
    ...
    const dreamTeams: Array<{ id: string; [key: string]: any }> = [];
    ...
    for (let i = 0; i < dreamTeams.length; i += BATCH_SIZE) {
      ...
      for (const dt of chunk) {
        ...
        const { total, breakdown, playerPoints } = calculateDreamTeamPoints(
          dt.players || [],
          rankings
        );
    ...
    const awardsPointsMapDT: Record<string, number> = {};
    ...
    const dtAllPointsMap: Record<string, { phase1: number; phase2: number }> = {};
    dtAllSnapForTotals.forEach((dtDoc) => {
      ...
      let uid = dtId;
      let ph = 1;
      if (dtId.endsWith("_phase1")) { uid = dtId.replace("_phase1", ""); ph = 1; }
      else if (dtId.endsWith("_phase2")) { uid = dtId.replace("_phase2", ""); ph = 2; }
      if (!dtAllPointsMap[uid]) dtAllPointsMap[uid] = { phase1: 0, phase2: 0 };
      if (ph === 1) dtAllPointsMap[uid].phase1 = dtPts;
...
function calculateDreamTeamPoints(
  dreamTeamPlayers: any[],
  rankings: Record<string, string[]>
): { total: number; breakdown: Record<string, number>; playerPoints: any[] } {
  let total = 0;
  const breakdown: Record<string, number> = { GK: 0, DF: 0, MF: 0, FW: 0 };
  ...
  dreamTeamPlayers.forEach((player) => {
    ...
    const rankIndex = list.findIndex(
      (rankedName: string) =>
        rankedName && rankedName.trim().toLowerCase() === name.trim().toLowerCase()
    );
